		...
		<section id="seccion-publicaciones-usuario">
			...
			<section id="personal-publicar">
				...
				?>
					...
					<textarea id="input-titulo-pub" maxlength="10" name="titulo" required></textarea>
			...
			<ul>
				<?php
					...
					$usuario=$_SESSION["usuario"];
					$resultado = $conn->query("SELECT `title`,`content`,`name`,`user`,`pic-user`,`time`,`numero` FROM `publicaciones` WHERE `user`='$usuario' ORDER BY `time` DESC");
					while($row = $resultado->fetch_assoc()) {
						$nombre=$row["name"];
						$user=$row["user"];
						$titulo=$row["title"];
						...
						$numero=$row["numero"];
						echo "<li>";
							echo "<section id='seccion-publicaciones-section'>";
								echo "<a href='actual.php' id='link-usuario-publicacion'>".$nombre."</a>";
								echo "<a href='actual.php'><img src='".$fotoUsuario."' width='50' height='50'/></a>";

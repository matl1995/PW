...
<html lang="es">
	...
	<head>
		...
		<link rel="stylesheet" href="css/estilo-publicaciones-portada.css">
		<link href="css/small.css" rel="stylesheet" media="(max-width: 500px)">
		...
		<section id="seccion-publicaciones">
			<ul>
				<?php
					...
					$resultado = $conn->query("SELECT `title`,`content`,`name`,`user`,`pic-user`,`time`,`numero` FROM `publicaciones` ORDER BY `time` DESC");
					while($row = $resultado->fetch_assoc()) {
						$nombre=$row["name"];
						$user=$row["user"];
						$titulo=$row["title"];
						$contenido=$row["content"];
						$contenido=substr($contenido, 0, 30);
						$fotoUsuario=$row["pic-user"];
						$tiempo=$row["time"];
						$numero=$row["numero"];
						echo "<li>";
							echo "<section>";
								echo "<a href='otro-usuario.php?usuario=".$user."' id='link-usuario-publicacion'>".$nombre."</a>";
								echo "<a href='otro-usuario.php?usuario=".$user."'><img src='".$fotoUsuario."' width='50' height='50'/></a>";
								echo "<p id='parrafo-tiempo-publicacion-p'>".$tiempo."</p>";
								echo "<a href='publicacion.php?publicacion=".$numero."&usuario=".$user."' id='link-titulo-publicacion'>".$titulo."</a>";
								echo "<p id='parrafo-contenido-publicacion-p'>".$contenido."</p>";
							echo "</section>";
						echo "</li>";
					}
				?>
